...
bool Model::saveBinary(const char *fileName) {
	FILE *file = fopen(fileName, "wb");
	if (!file) {
		return false;
	}
	MeshHeader header = {0};
	header.numPolygons = nIndices / 3;
	header.numVertices = streams[0].nVertices;
	uint nStreams = streams.getCount();
	const float* vertices = streams[0].vertices;

	Vector3D& minPoint = reinterpret_cast<Vector3D &>(header.boundingBox[0]);
	Vector3D& maxPoint = reinterpret_cast<Vector3D &>(header.boundingBox[3]);

	minPoint = Vector3D(FLT_MAX, FLT_MAX, FLT_MAX);
	maxPoint = Vector3D(FLT_MIN, FLT_MIN, FLT_MIN);
	for (uint i = 0; i < streams[0].nVertices; i++) {
		uint index = 3 * i;
		float x = streams[0].vertices[index];
		float y = streams[0].vertices[index];
		float z = streams[0].vertices[index];
		...
		//fwrite(streams[0].vertices, streams[i].nComponents * sizeof(float), streams[i].nVertices, file);
	}
	fwrite(&header, sizeof(header), 1, file);

	fwrite(streams[0].vertices, streams[0].nComponents * sizeof(float), streams[0].nVertices, file);

	Array<ushort> ushortIndices(nIndices);
	std::copy(streams[0].indices, streams[0].indices + nIndices, ushortIndices.getArray());

	fwrite(ushortIndices.getArray(), sizeof(ushort), nIndices, file);
		...
		switch (str[0]){
			case 'f':
				...
				while ((str = lineTok.next()) != NULL){
					...
					vtxIndices.add(atoi(str) - 1);

					if (texCoords.getCount() || normals.getCount()) lineTok.goToNext();
					if (texCoords.getCount()){
						txcIndices.add(atoi(lineTok.next()) - 1);
					}
					if (normals.getCount()){
						lineTok.goToNext();
						nrmIndices.add(atoi(lineTok.next()) - 1);
					}
...
struct Edge {
	Edge(const uint i0, const uint i1){
		index[0] = std::min(i0, i1);
		index[1] = std::max(i0, i1);
	}
...
void Model::drawSubBatch(Renderer *renderer, const uint batch, const uint first, const uint count){
	...
	int indexCount = std::min(count, batches[batch].nIndices - first);
