...
bool OpenGLApp::initAPI(){
  ...
  // HMONITOR hMonitor = MonitorFromWindow(m_handle->hWnd, MONITOR_DEFAULTTOPRIMARY);
	...
	int bestSamples = 0;
	uint nPFormats = 0;
	if (wglChoosePixelFormatARB(hdc, iAttribs, NULL, elementsOf(pixelFormats), pixelFormats, &nPFormats) && nPFormats > 0){
	...
	HGLRC newContext = wglCreateContextAttribsARB(hdc, NULL, formatAttributes);
	wglMakeCurrent(hdc, newContext);
	glContext = newContext;

	auto  DebugMessage = [](GLenum source, GLenum type, uint id, GLenum severity, GLsizei length,
		const char* message,
		const void* userParam)
	{
		assert(message && "NULL Pointer");
		const char* sourcesStr[] = {
			"OpenGL",
			"Window System",
			"Shader Compiler",
			"Third Party",
		...
		uint index = source - GL_DEBUG_SOURCE_API;
		assert(index < countof(sourcesStr) && "Out Of Range");
		const char* sourceMessage = sourcesStr[index];

		uint messageIndex = type - GL_DEBUG_TYPE_ERROR;
		assert(messageIndex < countof(messageTypes) && "Out Of Range");
		const char* messageType = messageTypes[messageIndex];

		if (severity != GL_DEBUG_SEVERITY_NOTIFICATION) {
			index = severity - GL_DEBUG_SEVERITY_HIGH;
		}
		else {
			index = 3;
		}
		assert(index < countof(Severity) && "Out Of Range");
		const char* severityStr = Severity[index];

		char buffer[1024];
		snprintf(buffer, countof(buffer), "OpenGL Debug Output:\nSource: %s\nType: %s\nid %d\nSeverity %s\nMessage: %s",
			sourceMessage, messageType, id, severityStr, message);
	...
	CheckOpenGLError();
	PFNGLDEBUGMESSAGECALLBACKARBPROC glDebugMessageCallback = reinterpret_cast<PFNGLDEBUGMESSAGECALLBACKARBPROC>(wglGetProcAddress("glDebugMessageCallback"));
