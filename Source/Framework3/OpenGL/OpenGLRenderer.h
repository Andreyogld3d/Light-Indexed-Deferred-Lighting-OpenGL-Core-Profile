...
public:
	...
	virtual unsigned int CreateUBO(ShaderID shader, const char* uboName) const;
	virtual void UpdateUBO(GLuint ubo, const void* data, size_t size) const;
	...
	virtual void updateBuffer(VertexBufferID vb, const void* data, size_t size);
	...
	BlendStateID addBlendStateSeperate(const int rgbSrcFactor, const int rgbDestFactor, const int alphaSrcFactor, const int alphaDestFactor, const int blendMode = BM_ADD, const int mask = ALL);
...
//	void setupFilter(const Texture &tex, const Filter filter, const uint flags);
	...
	GLenum currentRGBSrcFactor;
	GLenum currentRGBDstFactor;
	GLenum currentAlphaSrcFactor;
	GLenum currentAlphaDstFactor;
	...
	static const int TEXCOORD_ATTRIB = 8;
	static const int NORMAL_ATTRIB = 4;
